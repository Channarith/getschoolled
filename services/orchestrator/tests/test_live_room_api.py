...
def _fake_ask_stream_events(*_a, **_k):
    # Mirror TeachingSessions.ask_stream: incremental deltas then a final done
    # event whose text is the whole guarded answer.
...
def test_ask_stream_does_not_duplicate_final_answer(monkeypatch):
    # Regression: the ask-stream generator used to append the done event's full
    # text on top of the token deltas, doubling the reply in the posted host
    # message and TTS ("Alpha Beta.Alpha Beta.").
    import orchestrator.main as m

    monkeypatch.setattr(m.get_sessions(), "ask_stream", _fake_ask_stream_events)
    info = _start_salareen_class(6)
    room_id = info["room_id"]
    client.post(f"/api/live-rooms/{room_id}/start-presentation", json={})
    pid = client.post(
        f"/api/live-rooms/{room_id}/join",
        json={"name": "Ada", "identity": "ada-dup"},
    ).json()["participant"]["id"]
    ...
    assert done["text"] == "Alpha Beta.", done
    host_text = done["host_message"]["text"]
    assert host_text == "@Ada Alpha Beta. Does that answer your question?", host_text
    assert "Alpha Beta.Alpha" not in host_text


def test_chat_theodore_reply_posts_answer_not_event_dicts(monkeypatch):
    # Regression: the chat auto-reply used to str()-join the ask_stream event
    # dicts, posting a message full of "{'type': 'delta', ...}" reprs instead of
    # the actual answer.
    import orchestrator.main as m
    ...
    pid = client.post(
        f"/api/live-rooms/{room_id}/join",
        json={"name": "Ada", "identity": "ada-chat"},
    ).json()["participant"]["id"]

    client.post(
        f"/api/live-rooms/{room_id}/chat",
        json={"participant_id": pid, "text": "Hi Theodore!"},
    )

    chat = client.get(f"/api/live-rooms/{room_id}").json()["chat"]
    texts = [m_["text"] for m_ in chat]
    assert "Alpha Beta." in texts, texts
    assert all("{'type'" not in t and "'text':" not in t for t in texts), texts
