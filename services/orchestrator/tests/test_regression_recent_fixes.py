...
# ---------------------------------------------------------------------------
# v0.45.12 — Admin flags: X-Admin-Secret auth must be enforced
# ---------------------------------------------------------------------------

def test_admin_flags_wrong_secret_rejected():
    """Regression v0.45.12: the memory-service /admin/flags endpoint must
    reject an incorrect X-Admin-Secret with 401."""
    from memory.main import app as mem_app
    from fastapi.testclient import TestClient as TC
    mem_client = TC(mem_app)
    r = mem_client.get("/admin/flags", headers={"X-Admin-Secret": "wrong"})
    assert r.status_code == 401, (
        "incorrect admin secret must be rejected — "
        "a permissive BFF is a regression of v0.45.12"
    )


def test_admin_flags_correct_secret_accepted():
    """Regression v0.45.12: correct X-Admin-Secret returns flag list."""
    from memory.main import app as mem_app
    from fastapi.testclient import TestClient as TC
    mem_client = TC(mem_app)
    r = mem_client.get("/admin/flags", headers={"X-Admin-Secret": "dev-admin-secret"})
    assert r.status_code == 200
    assert "flags" in r.json()


def test_admin_flags_no_secret_returns_401():
    """Regression v0.45.12: no secret header must return 401."""
    from memory.main import app as mem_app
    from fastapi.testclient import TestClient as TC
    mem_client = TC(mem_app)
    r = mem_client.get("/admin/flags")
    assert r.status_code == 401


# ---------------------------------------------------------------------------
# v0.45.13 — Survey once and done: server 409 on duplicate
# ---------------------------------------------------------------------------

def test_survey_duplicate_rejected_by_server():
    """Regression v0.45.13: same student submitting survey twice for same
    course must be rejected — frontend localStorage guard is the first line,
    server 409 is the fallback."""
    from memory.main import app as mem_app
    from aoep_shared.survey import SurveyStore
    from fastapi.testclient import TestClient as TC
    mem_client = TC(mem_app)
    mem_app.state.surveys = SurveyStore()

    payload = {"course_id": "dup-course", "overall": 4, "student_id": "stu-once"}
    first = mem_client.post("/survey/post-class", json=payload)
    assert first.status_code == 200

    second = mem_client.post("/survey/post-class", json=payload)
    assert second.status_code == 409, (
        "server must reject duplicate survey — "
        "this is the backend enforcement of once-per-account behaviour"
    )


# ---------------------------------------------------------------------------
# v0.45.14 — Sales demo flag default false
# ---------------------------------------------------------------------------

def test_sales_demo_flag_not_enabled_by_default():
    """Regression v0.45.14: sales_demo.enabled must be off by default so the
    floating Sales Demo button doesn't appear for all users."""
    from memory.main import app as mem_app
    from aoep_shared.flags import FlagStore
    from fastapi.testclient import TestClient as TC
    mem_client = TC(mem_app)
    mem_app.state.flags = FlagStore()  # fresh store — no flags set

    r = mem_client.get("/flags/evaluate", params={"keys": "sales_demo.enabled"})
    # Either 200 with enabled=False, or the key just isn't present.
    if r.status_code == 200:
        body = r.json()
        val = body.get("sales_demo.enabled", {}).get("enabled", False)
        assert val is False, (
            "sales_demo.enabled must default to False; "
            "a True default shows the Sales Demo button to all users"
        )
