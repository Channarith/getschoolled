...
def render_rag_lab() -> None:
    _lab_console(
        ...
        subtitle="RAG auto-tune  ·  live trusted current awareness  ·  :8095",
        accent="#7DD3FC",
        left_title="Current awareness",
        left_rows=[
            ("query_route", "current"),
            ("as_of", "2026-08-22 UTC"),
            ("source_policy", "trusted-balanced"),
            ("publisher_dedup", "on"),
            ("sensitive_min", "2 sources"),
            ("article_storage", "transient"),
            ("grounding_guard", "required"),
            ("fallback", "do not guess"),
        ],
        right_title="Live evidence console",
        right_lines=[
            "$ uvicorn theodore_rag_lab.main:app --port 8095",
            "# POST /api/current-awareness/research",
            "→ who is the current U.S. president?",
            "→ route=current  as_of=10:42Z",
            "→ Reuters + AP corroborated",
            "# what conflicts are happening globally?",
            "→ sensitive=true  independent_sources=3",
            "→ article text read transiently",
            "→ citations linked  ·  model memory blocked",
        ],
        pills=["live search", "trusted sources", "citation-backed"],
    )
...
def render_music_lab() -> None:
    _lab_console(
        ...
        subtitle="Fast first-song lazy load  ·  karaoke theater  ·  pause & ask  ·  :8097",
        accent="#C4B5FD",
        left_title="Player",
        left_rows=[
            ("featured MP3s", "10 · first song loads immediately"),
            ("lazy loading", "1 card at a time during idle"),
