...
MONITOR_JS = (
    ...
    function pickOwnerFace(faces, nowMs) {
      ...
      const best = bestFaceIndex(faces, faceOwnerState, OWNER_MATCH_SCORE_MIN);
      if (best.index >= 0) {
        const box = faceBoxFromPts(faces[best.index]);
        if (box) {
          ...
          faceOwnerState.lastBox = lb
            ...
            : box;
          const fp = faceFingerprint(faces[best.index]);
        ...
        faceOwnerState.matchScore = best.score;
        return {
          index: best.index, owner_enrolled: true, owner_match: true, match_score: best.score,
          secondary_count: Math.max(0, faceCount - 1), face_count: faceCount,
